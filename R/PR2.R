...
annotate_pr2_with_dbs <- function(tax_path,
    ...
    if (!file.exists(tax_path)) {
        stop("File .tax non trovato: ", tax_path, call. = FALSE)
    }
    ...
    message("=== Step 1: Parsing PR2 .tax ===")
...
# ============================================================
# 5. ANNOTAZIONE DI TUTTI I FILE *.tax IN UNA CARTELLA
# ============================================================

annotate_all_pr2_tax <- function(input_dir = ".",
                                 db_names,
                                 db_dir     = "inst/db/",
                                 output_dir = "annotated_results/",
                                 recursive  = FALSE) {
    ...
    if (length(tax_files) == 0) {
        stop(
            "Nessun file *.tax trovato nella cartella: ",
    ...
    message(
        "\nTrovati ", length(tax_files),
        " file *.tax in: ",
        normalizePath(input_dir, winslash = "/", mustWork = FALSE)
    )

    all_results <- lapply(tax_files, function(tax_path) {

        tryCatch(
            annotate_pr2_with_dbs(
                tax_path   = tax_path,
                db_names   = db_names,
                db_dir     = db_dir,
    ...
    summary_file <- file.path(output_dir, "pr2_annotation_summary.tsv")
